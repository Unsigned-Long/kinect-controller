...
void DK_Camera::doOne() {
  ...
  if (save_img_flag_) { // 000_1_000.jpg
...
bool DK_Camera::stop() {
  ...
  std_msgs::String str;
  // flag:savePath
  str.data = std::to_string(ns_runtime_flags::INFO_MESSAGE_TEXTVIEW) +
             ":" + "data saved to " + _saveFolder.data;
...
bool DK_Camera::start() {
  ...
  while (ros::ok()) {
    ...
    std_msgs::String curColorImgName, curDepthImgName, curTimeStamp, runTimeMsg;
    ...
    runTimeMsg.data = std::to_string(ns_runtime_flags::COLOR_DEPTH_TIME) + ":" +
