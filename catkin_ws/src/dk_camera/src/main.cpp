...
int main(int argc, char **argv) {
  ...
  while (ros::ok()) {
    if (k4a::device::get_installed_count() == 0) {
      ...
      std_msgs::String str;
      str.data = std::to_string(ns_runtime_flags::INFO_MESSAGE_TEXTVIEW) + ":" + "No Vaild Data Gatherer";
  ...
  ROS_INFO("'%s'", "vaild device connected");
  std_msgs::String str;
  str.data = std::to_string(ns_runtime_flags::INFO_MESSAGE_TEXTVIEW) + ":" + "Vaild Data Gatherer Connected";
  runtime_pub.publish(str);
  ...
  try {
    ...
    std_msgs::String str;
    str.data = std::to_string(ns_runtime_flags::INFO_MESSAGE_TEXTVIEW) + ":" + err.what();
    runtime_pub.publish(str);
...
void camera_control_callback(const std_msgs::String::ConstPtr &msg) {
  ...
  if (camera == nullptr) {
    std_msgs::String str;
    if (data == "start") {
      str.data = std::to_string(ns_runtime_flags::INFO_MESSAGE_TOAST) + ":" + "please connect to data gatherer";
    } else if (data == "stop") {
      str.data = std::to_string(ns_runtime_flags::INFO_MESSAGE_TOAST) + ":" + "data gatherer is not working";
    }
    runtime_pub.publish(str);
    return;
  }
  ...
  if (data == "start" && camera->stop_flag_ == true) {
    if (camera->stop_flag_ == false) {
      std_msgs::String str;
      str.data = std::to_string(ns_runtime_flags::INFO_MESSAGE_TOAST) + ":" + "data gatherer is working";
      runtime_pub.publish(str);
    } else {
